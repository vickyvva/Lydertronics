...
// Open when clicking icon
document.getElementById("chat-toggle").onclick = toggleChat;

// Send message (your existing function improved)
async function sendMessage() {
  const input = document.getElementById("input");
  const message = input.value;
  ...
  messages.innerHTML += `<div style="text-align:right; margin:5px;"><b>You:</b> ${message}</div>`;
  input.value = "";
  ...
  try {
    ...
    messages.innerHTML += `<div style="margin:5px;"><b>Bot:</b> ${data.reply}</div>`;
    messages.scrollTop = messages.scrollHeight;

  } catch (error) {
    messages.innerHTML += `<div style="color:red;">Error connecting</div>`;
  }
}
